...
impl Row {
    ...
    /// Read a column as `f64`. Integers coerce, because SQLite may store a whole
    /// REAL as INTEGER. `None` for NULL, text, or a missing column.
...
const INDEXES: &[&str] = &[
    ...
    "CREATE INDEX IF NOT EXISTS idx_spots_available ON Spots(is_available, deleted_at)",
...
/// Columns an older `Spots` table may be missing.
///
/// `NOT NULL` additions carry a default so `ALTER TABLE` is legal on a populated
/// table. `deleted_at` stays nullable: NULL means the listing is live.
const SPOTS_COLUMNS: &[(&str, &str)] = &[
...
/// Columns an older `Bookings` table may be missing.
const BOOKINGS_COLUMNS: &[(&str, &str)] = &[
    ("spot_id", "TEXT NOT NULL DEFAULT ''"),
    ("user_id", "TEXT NOT NULL DEFAULT ''"),
    ("time_in", "INTEGER NOT NULL DEFAULT 0"),
...
fn ensure_schema(db: &Db) -> Result<(), DbError> {
    ...
    add_missing_columns(db, "Spots", SPOTS_COLUMNS)?;
    add_missing_columns(db, "Bookings", BOOKINGS_COLUMNS)?;
    for statement in INDEXES {
        db.exec(statement)?;
    }
    seed_spots_if_empty(db)?;
    Ok(())
}
...
/// Parking listing stored in `Spots`. JSON uses camelCase at the HTTP edge.
#[derive(Debug, Clone, PartialEq)]
pub struct Spot {
    /// Spot id, the `_id` column.
    pub id: String,
    /// Owner user id. Seed rows use `seed-host`.
    pub user_id: String,
    /// Listing title.
    pub title: String,
    /// Street address.
    ...
    pub price: f64,
    /// Overnight rate in dollars.
    pub overnight: f64,
    /// `Street`, `Driveway`, or `Garage`.
    pub style: String,
    /// Host-written description.
    pub summary: String,
    /// Whether drivers can book the spot.
...
/// Fields written on create or after a patch is merged with the stored row.
#[derive(Debug, Clone, PartialEq)]
pub struct SpotWrite {
    /// Listing title.
    pub title: String,
    /// Street address.
    pub street_address: String,
...
/// Filters for [`Pool::list_spots`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SpotFilter {
    /// When true, only rows with `is_available = 1` are returned.
    pub available_only: bool,
    /// When set, only that owner's listings are returned.
    pub user_id: Option<String>,
}

/// Parking booking stored in `Bookings`.
#[derive(Debug, Clone, PartialEq)]
pub struct Booking {
    /// Booking id, the `_id` column.
    pub id: String,
    /// Spot that was booked.
    pub spot_id: String,
    /// Renter user id.
    pub user_id: String,
    ...
    pub time_out: i64,
    /// Charged total in dollars.
    pub total: f64,
    /// Booking status. Creates write `confirmed`.
    pub status: String,
    /// Unix milliseconds when the booking was inserted.
    pub created_at: i64,
}
...
/// Demo San Francisco inventory inserted when `Spots` is empty.
const SEED_SPOTS: &[SeedSpot] = &[
    SeedSpot {
        id: "seed-marina",
        title: "Marina Driveway",
        street: "2100 Chestnut St",
        city: "San Francisco",
        state: "CA",
        zip: "94123",
        latitude: 37.8005,
        ...
        overnight: 25.0,
        style: "Driveway",
        summary: "Quiet residential driveway two blocks from the Marina Green. Level surface, easy in and out, perfect for sedans and small SUVs.",
    },
    SeedSpot {
        id: "seed-soma",
        title: "SOMA Garage Spot",
        street: "450 Folsom St",
        city: "San Francisco",
        state: "CA",
        zip: "94105",
        latitude: 37.7872,
        ...
        overnight: 35.0,
        style: "Garage",
        summary: "Covered garage stall in SOMA near the bridge approach. Security gate, well lit, ideal for weekday office parkers.",
    },
    SeedSpot {
        id: "seed-mission",
        title: "Mission Street Curb",
        street: "2200 Mission St",
        city: "San Francisco",
        state: "CA",
        zip: "94110",
        latitude: 37.7609,
        ...
        overnight: 18.0,
        style: "Street",
        summary: "Legal street space next to a cafe in the Mission. Short walk to BART. Clear of street sweeping on listed days.",
    },
    SeedSpot {
        id: "seed-north-beach",
        title: "North Beach Alley Spot",
        street: "15 Divisadero St",
        city: "San Francisco",
        state: "CA",
        zip: "94117",
        latitude: 37.7706,
        ...
        overnight: 20.0,
        style: "Driveway",
        summary: "Private driveway near Divisadero corridor. Spacious enough for a midsize car. Host lives on site and can unlock gate.",
    },
    SeedSpot {
        id: "seed-fidi",
        ...
        overnight: 40.0,
        style: "Garage",
        summary: "Downtown covered garage with elevator access. Best for daytime meetings near Embarcadero. Height limit 6ft 8in.",
    },
];

/// Map an unknown stored style to `Street`, matching the Node row mapper.
fn canonical_style(style: &str) -> String {
    if SPOT_STYLES.contains(&style) {
        style.to_string()
    } else {
        "Street".to_string()
    }
...
/// Rebuild a [`Spot`] from a `SELECT *` row. `None` when a required column is missing.
fn spot_from_row(row: &Row) -> Option<Spot> {
    Some(Spot {
        id: row.text("_id")?.to_string(),
        user_id: row.text("user_id")?.to_string(),
        title: row.text("title")?.to_string(),
        street_address: row.text("street_address")?.to_string(),
        city: row.text("city")?.to_string(),
        state: row.text("state")?.to_string(),
        zipcode: row.text("zipcode")?.to_string(),
        latitude: row.real("latitude")?,
        longitude: row.real("longitude")?,
        price: row.real("price")?,
        overnight: row.real("overnight")?,
        style: canonical_style(row.text("style")?),
        summary: row.text("summary")?.to_string(),
        is_available: row.int("is_available")? != 0,
        created_at: row.int("created_at")?,
        updated_at: row.int("updated_at")?,
    })
}

/// Rebuild a [`Booking`] from a `SELECT *` row.
fn booking_from_row(row: &Row) -> Option<Booking> {
    Some(Booking {
        id: row.text("_id")?.to_string(),
        spot_id: row.text("spot_id")?.to_string(),
        user_id: row.text("user_id")?.to_string(),
        time_in: row.int("time_in")?,
        time_out: row.int("time_out")?,
        total: row.real("total")?,
        status: row.text("status").unwrap_or("confirmed").to_string(),
        created_at: row.int("created_at")?,
    })
}

fn get_spot_tx(db: &Db, id: &str) -> Result<Option<Spot>, DbError> {
    let rows = db.query(
        "SELECT * FROM Spots WHERE _id = ? AND deleted_at IS NULL",
        &[Value::Text(id.to_string())],
    )?;
    Ok(rows.first().and_then(spot_from_row))
}

fn insert_seed_spot(db: &Db, spot: &SeedSpot, now: i64) -> Result<(), DbError> {
    db.run(
        "INSERT INTO Spots (
        ...
        &[
            Value::Text(spot.id.to_string()),
            Value::Text(SEED_HOST_ID.to_string()),
            Value::Text(spot.title.to_string()),
            Value::Text(spot.street.to_string()),
            Value::Text(spot.city.to_string()),
            Value::Text(spot.state.to_string()),
            Value::Text(spot.zip.to_string()),
            Value::Real(spot.latitude),
            Value::Real(spot.longitude),
            Value::Real(spot.price),
            Value::Real(spot.overnight),
            Value::Text(spot.style.to_string()),
            Value::Text(spot.summary.to_string()),
            Value::Int(now),
            Value::Int(now),
        ],
    )?;
...
/// Insert the demo inventory once. A non-empty table is left alone.
fn seed_spots_if_empty(db: &Db) -> Result<(), DbError> {
    let rows = db.query("SELECT COUNT(*) AS c FROM Spots", &[])?;
    let count = rows.first().and_then(|row| row.int("c")).unwrap_or(0);
    if count > 0 {
        return Ok(());
    }
    let now = unix_ms();
    for spot in SEED_SPOTS {
        insert_seed_spot(db, spot, now)?;
    }
    Ok(())
}
...
impl Pool {
    ...
    /// List non-deleted spots, cheapest first.
    ///
    /// # Errors
    /// Returns [`DbError`] on a driver failure.
    pub fn list_spots(&self, filter: &SpotFilter) -> Result<Vec<Spot>, DbError> {
        let mut sql = String::from("SELECT * FROM Spots WHERE deleted_at IS NULL");
        ...
        self.with(|db| {
            ...
            Ok(rows.iter().filter_map(spot_from_row).collect())
        })
    }

    /// Fetch one non-deleted spot by id.
    ///
    /// # Errors
    /// Returns [`DbError`] on a driver failure.
    pub fn get_spot(&self, id: &str) -> Result<Option<Spot>, DbError> {
        self.with(|db| get_spot_tx(db, id))
    }

    /// Insert a spot owned by `user_id` and return the stored row.
    ///
    /// # Errors
    /// Returns [`DbError`] on a driver failure or when the new row cannot be read back.
    pub fn insert_spot(
        &self,
        id: &str,
        user_id: &str,
        ...
        self.with(|db| {
            db.run(
                ...
                &spot_params(id, user_id, write, now_ms),
            )?;
            get_spot_tx(db, id)?.ok_or_else(|| DbError::local("failed to load created spot"))
        })
    }

    /// Replace a spot's fields when `user_id` owns it. Other callers get `None`.
    ///
    /// The `WHERE` clause filters on the caller id so a guessed id cannot change
    ...
    pub fn update_spot(
        ...
        self.with(|db| {
            let changes = db.run(
                ...
                &update_params(id, user_id, write, now_ms),
            )?;
            if changes.changes == 0 {
                return Ok(None);
            }
    ...
    /// Soft-delete a spot owned by `user_id`. Returns false when nothing matched.
    ///
    /// # Errors
    /// Returns [`DbError`] on a driver failure.
    pub fn delete_spot(&self, id: &str, user_id: &str, now_ms: i64) -> Result<bool, DbError> {
        self.with(|db| {
            let changes = db.run(
                "UPDATE Spots SET deleted_at = ?, is_available = 0, updated_at = ?
                 WHERE _id = ? AND user_id = ? AND deleted_at IS NULL",
                &[
                    Value::Int(now_ms),
                    Value::Int(now_ms),
    ...
    /// Insert a confirmed booking and return it.
    ///
    /// # Errors
    /// Returns [`DbError`] on a driver failure.
    pub fn insert_booking(&self, booking: &Booking) -> Result<(), DbError> {
        self.with(|db| {
            db.run(
                "INSERT INTO Bookings (
                    _id, spot_id, user_id, time_in, time_out, total, status, created_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                &[
                    Value::Text(booking.id.clone()),
                    Value::Text(booking.spot_id.clone()),
                    Value::Text(booking.user_id.clone()),
                    Value::Int(booking.time_in),
                    Value::Int(booking.time_out),
                    Value::Real(booking.total),
                    Value::Text(booking.status.clone()),
                    Value::Int(booking.created_at),
                ],
            )?;
            Ok(())
        })
    ...
    /// List a renter's bookings, newest first, with the spot when it still exists.
    ///
    /// # Errors
    /// Returns [`DbError`] on a driver failure.
    pub fn list_bookings_for_user(
        &self,
        user_id: &str,
    ) -> Result<Vec<(Booking, Option<Spot>)>, DbError> {
        self.with(|db| {
            let rows = db.query(
            ...
            let mut out = Vec::new();
            for row in &rows {
                let Some(booking) = booking_from_row(row) else {
                    continue;
                };
                let spot = get_spot_tx(db, &booking.spot_id)?;
                out.push((booking, spot));
            }
            Ok(out)
        })
    }
...
fn spot_params(id: &str, user_id: &str, write: &SpotWrite, now_ms: i64) -> Vec<Value> {
    vec![
        Value::Text(id.to_string()),
        Value::Text(user_id.to_string()),
        Value::Text(write.title.clone()),
        Value::Text(write.street_address.clone()),
        Value::Text(write.city.clone()),
        Value::Text(write.state.clone()),
        Value::Text(write.zipcode.clone()),
        Value::Real(write.latitude),
        Value::Real(write.longitude),
        Value::Real(write.price),
        Value::Real(write.overnight),
        Value::Text(write.style.clone()),
        Value::Text(write.summary.clone()),
        Value::Int(i64::from(write.is_available)),
        Value::Int(now_ms),
        Value::Int(now_ms),
    ]
}

fn update_params(id: &str, user_id: &str, write: &SpotWrite, now_ms: i64) -> Vec<Value> {
    vec![
        Value::Text(write.title.clone()),
        Value::Text(write.street_address.clone()),
        Value::Text(write.city.clone()),
...
mod tests {
    ...
    fn opens_pool_and_creates_schema() {
        ...
        assert_eq!(
            names,
            vec!["Auths", "Bookings", "Spots", "Users", "WebhookEvents"]
        );
    }

    #[test]
    fn update_and_delete_require_the_owner() {
        let tmp = TempDb::new("spots-owner");
        let pool = Pool::open(&tmp.path(), 1).expect("pool");
        let write = SpotWrite {
            title: "Owned Driveway".into(),
            street_address: "10 Oak St".into(),
            ...
            overnight: 15.0,
            style: "Driveway".into(),
            summary: "A private driveway long enough for a sedan, off the main road.".into(),
            is_available: true,
        };
        let created = pool.insert_spot("spot-1", "owner", &write, 10).expect("insert");
        assert_eq!(created.user_id, "owner");
        assert!(pool.update_spot("spot-1", "stranger", &write, 11).unwrap().is_none());
        assert!(!pool.delete_spot("spot-1", "stranger", 12).unwrap());
