...
fn dispatch(state: &AppState, req: &Request) -> Response {
    match (req.method.as_str(), req.path.as_str()) {
        ...
        ("GET", "/api/spots") => spots_list(state, req),
        ("GET", "/api/spots/mine") => spots_mine(state, req),
        ("POST", "/api/spots") => spots_create(state, req),
        ("GET", "/api/bookings") => bookings_list(state, req),
        ("POST", "/api/bookings") => bookings_create(state, req),
        ("GET", path) if spot_path_id(path).is_some() => spots_get(state, req),
        ("PUT", path) if spot_path_id(path).is_some() => spots_update(state, req),
        ("DELETE", path) if spot_path_id(path).is_some() => spots_delete(state, req),
...
const SPOT_STYLES: &[&str] = &["Street", "Driveway", "Garage"];
const MIN_TITLE_CHARS: usize = 2;
const MIN_STREET_CHARS: usize = 3;
const MIN_SUMMARY_CHARS: usize = 30;
const MAX_HOURLY_PRICE: f64 = 100.0;
const MAX_OVERNIGHT: f64 = 500.0;
const MS_PER_HOUR: f64 = 3_600_000.0;
const MIN_BILLABLE_HOURS: f64 = 0.25;
const MAX_BOOKING_MS: i64 = 7 * 24 * 60 * 60 * 1000;

/// Id segment of `/api/spots/:id`. Empty and nested paths are not ids.
///
/// `/api/spots/mine` is a literal route registered ahead of this pattern, so a
/// GET for that path never reaches here.
fn spot_path_id(path: &str) -> Option<&str> {
    let id = path.strip_prefix("/api/spots/")?;
...
fn authed_mutate(state: &AppState, req: &Request) -> Result<String, Response> {
    let user_id = require_auth(state, req)?;
    require_csrf(state, req, &user_id)?;
    Ok(user_id)
}

fn spots_db_err(state: &AppState, context: &str, public: &str, err: &db::DbError) -> Response {
    state
        .log
        .error(context, &[("error", json::s(err.to_string()))]);
    err_json(500, public)
}

fn spots_list(state: &AppState, req: &Request) -> Response {
    if let Err(res) = require_auth(state, req) {
        return res;
    }
    let available_only = req.query_param("available").as_deref() != Some("0");
    match state.pool.list_spots(&db::SpotFilter {
        available_only,
        user_id: None,
    }) {
        Ok(spots) => json_res(200, &spots_json(&spots)),
        Err(err) => spots_db_err(state, "List spots failed", "Failed to list spots", &err),
    }
}

fn spots_mine(state: &AppState, req: &Request) -> Response {
    let user_id = match require_auth(state, req) {
        Ok(id) => id,
        Err(res) => return res,
    };
    match state.pool.list_spots(&db::SpotFilter {
        ...
        Ok(spots) => json_res(200, &spots_json(&spots)),
        Err(err) => spots_db_err(state, "List my spots failed", "Failed to list your spots", &err),
    }
}

fn spots_get(state: &AppState, req: &Request) -> Response {
    if let Err(res) = require_auth(state, req) {
        return res;
    }
    let Some(id) = spot_path_id(&req.path) else {
        return err_json(400, "Spot id required");
    };
    match state.pool.get_spot(id) {
        Ok(Some(spot)) => json_res(200, &spot_json(&spot)),
        Ok(None) => err_json(404, "Spot not found"),
        Err(err) => spots_db_err(state, "Get spot failed", "Failed to get spot", &err),
    }
}

fn spots_create(state: &AppState, req: &Request) -> Response {
    let user_id = match authed_mutate(state, req) {
        Ok(id) => id,
        Err(res) => return res,
    };
    let body = match parse_json_body(req) {
    ...
    let write = match spot_from_body(&body, None) {
        Ok(write) => write,
    ...
    match state.pool.insert_spot(&id, &user_id, &write, config::now_ms()) {
        Ok(spot) => json_res(201, &spot_json(&spot)),
        Err(err) => spots_db_err(state, "Create spot failed", "Failed to create spot", &err),
    }
}

fn spots_update(state: &AppState, req: &Request) -> Response {
    let user_id = match authed_mutate(state, req) {
        Ok(id) => id,
        Err(res) => return res,
    };
    ...
    let existing = match state.pool.get_spot(id) {
        Ok(Some(spot)) if spot.user_id == user_id => spot,
        Ok(_) => return err_json(404, "Spot not found or not owned by you"),
        Err(err) => return spots_db_err(state, "Update spot failed", "Failed to update spot", &err),
    };
    ...
    match state
        .pool
        .update_spot(id, &user_id, &write, config::now_ms())
    {
        Ok(Some(spot)) => json_res(200, &spot_json(&spot)),
        Ok(None) => err_json(404, "Spot not found or not owned by you"),
        Err(err) => spots_db_err(state, "Update spot failed", "Failed to update spot", &err),
    }
}

fn spots_delete(state: &AppState, req: &Request) -> Response {
    let user_id = match authed_mutate(state, req) {
        Ok(id) => id,
        Err(res) => return res,
    };
    ...
    match state.pool.delete_spot(id, &user_id, config::now_ms()) {
        Ok(true) => Response::empty(204),
        Ok(false) => err_json(404, "Spot not found or not owned by you"),
        Err(err) => spots_db_err(state, "Delete spot failed", "Failed to delete spot", &err),
    }
}

fn bookings_list(state: &AppState, req: &Request) -> Response {
    let user_id = match require_auth(state, req) {
        Ok(id) => id,
        Err(res) => return res,
    };
    match state.pool.list_bookings_for_user(&user_id) {
        Ok(rows) => {
            let items: Vec<Json> = rows
                .iter()
                .map(|(booking, spot)| booking_json(booking, spot.as_ref()))
                .collect();
            json_res(200, &Json::Arr(items))
        }
        Err(err) => spots_db_err(state, "List bookings failed", "Failed to list bookings", &err),
    }
}

fn bookings_create(state: &AppState, req: &Request) -> Response {
    let user_id = match authed_mutate(state, req) {
        Ok(id) => id,
        Err(res) => return res,
    };
    ...
    let (spot_id, time_in, time_out) = match booking_window(&body) {
        Ok(window) => window,
        Err(message) => return err_json(400, message),
    };
    ...
    let spot = match state.pool.get_spot(&spot_id) {
        ...
        Err(err) => {
            return spots_db_err(state, "Create booking failed", "Failed to create booking", &err)
        }
    };
    if !spot.is_available {
        return err_json(400, "Spot is not available");
    }
    let id = match generate_uuid() {
        Ok(id) => id,
        Err(res) => return res,
    ...
    let booking = db::Booking {
        ...
        time_in,
        time_out,
        total: booking_total(spot.price, time_in, time_out),
        status: "confirmed".to_string(),
        created_at: config::now_ms(),
    };
    if let Err(err) = state.pool.insert_booking(&booking) {
        return spots_db_err(state, "Create booking failed", "Failed to create booking", &err);
    }
    json_res(201, &booking_json(&booking, Some(&spot)))
}
...
fn read_spot_draft(body: &Json, existing: Option<&db::Spot>) -> SpotDraft {
    let fallback = existing.cloned();
    SpotDraft {
        title: merged_text(body, "title", fallback.as_ref().map(|s| s.title.as_str())),
        street_address: merged_text(
            body,
            "streetAddress",
            fallback.as_ref().map(|s| s.street_address.as_str()),
        ),
        city: merged_text(body, "city", fallback.as_ref().map(|s| s.city.as_str())),
        state: merged_text(body, "state", fallback.as_ref().map(|s| s.state.as_str())),
        zipcode: merged_zip(body, fallback.as_ref().map(|s| s.zipcode.as_str())),
        latitude: merged_number(body, "latitude", fallback.as_ref().map(|s| s.latitude)),
        longitude: merged_number(body, "longitude", fallback.as_ref().map(|s| s.longitude)),
        price: merged_number(body, "price", fallback.as_ref().map(|s| s.price)),
        overnight: merged_number(body, "overnight", fallback.as_ref().map(|s| s.overnight)),
        style: merged_text(body, "style", fallback.as_ref().map(|s| s.style.as_str())),
        summary: merged_text(body, "summary", fallback.as_ref().map(|s| s.summary.as_str())),
        is_available: merged_available(
            body,
            fallback.as_ref().map(|s| s.is_available).unwrap_or(true),
        ),
    }
}

fn spot_from_body(body: &Json, existing: Option<&db::Spot>) -> Result<db::SpotWrite, &'static str> {
    let draft = read_spot_draft(body, existing);
    let place = validate_spot_place(&draft)?;
    let rates = validate_spot_rates(&draft)?;
    Ok(db::SpotWrite {
        title: place.0,
        street_address: place.1,
        city: place.2,
        state: place.3,
        zipcode: place.4,
        latitude: rates.0,
...
fn validate_spot_place(draft: &SpotDraft) -> Result<(String, String, String, String, String), &'static str> {
    if draft.title.len() < MIN_TITLE_CHARS {
        return Err("Title is required");
    }
    if draft.street_address.len() < MIN_STREET_CHARS {
        return Err("Street address is required");
    }
    if !is_alpha_space(&draft.city) {
        return Err("City is invalid");
    }
    if !is_alpha_space(&draft.state) {
        return Err("State is invalid");
    }
    if !is_zip(&draft.zipcode) {
        return Err("Zipcode should be 12345 or 12345-1234");
    }
    Ok((
        draft.title.clone(),
        draft.street_address.clone(),
        draft.city.clone(),
        draft.state.clone(),
        draft.zipcode.clone(),
    ))
...
fn validate_spot_rates(draft: &SpotDraft) -> Result<(f64, f64, f64, f64), &'static str> {
    let latitude = draft.latitude.ok_or("Latitude must be between -90 and 90")?;
    if !(-90.0..=90.0).contains(&latitude) {
        return Err("Latitude must be between -90 and 90");
    }
    let longitude = draft.longitude.ok_or("Longitude must be between -180 and 180")?;
    if !(-180.0..=180.0).contains(&longitude) {
        return Err("Longitude must be between -180 and 180");
    }
    let price = draft.price.ok_or("Price must be greater than 0 and less than 100")?;
    if !(price > 0.0 && price < MAX_HOURLY_PRICE) {
        return Err("Price must be greater than 0 and less than 100");
    }
    let overnight = draft.overnight.ok_or("Overnight rate must be 0\u{2013}500")?;
    if overnight < 0.0 || overnight >= MAX_OVERNIGHT {
        return Err("Overnight rate must be 0\u{2013}500");
    }
    if !SPOT_STYLES.contains(&draft.style.as_str()) {
        return Err("Style must be one of: Street, Driveway, Garage");
    }
    if draft.summary.len() < MIN_SUMMARY_CHARS {
        return Err("Summary must be at least 30 characters");
    }
    Ok((latitude, longitude, price, overnight))
}

fn merged_text(body: &Json, key: &str, fallback: Option<&str>) -> String {
    match body.get(key) {
        Some(Json::Str(value)) => value.trim().to_string(),
        Some(Json::Null) | None => fallback.unwrap_or("").to_string(),
        Some(_) => String::new(),
    }
}

fn merged_zip(body: &Json, fallback: Option<&str>) -> String {
    match body.get("zipcode") {
        Some(Json::Str(value)) => value.trim().to_string(),
        Some(Json::Num(value)) if value.fract() == 0.0 && value.is_finite() => {
            format!("{}", *value as i64)
        }
        Some(Json::Null) | None => fallback.unwrap_or("").to_string(),
        Some(_) => String::new(),
...
fn merged_number(body: &Json, key: &str, fallback: Option<f64>) -> Option<f64> {
    match body.get(key) {
...
fn merged_available(body: &Json, fallback: bool) -> bool {
    match body.get("isAvailable") {
        Some(Json::Bool(value)) => *value,
        _ => fallback,
    }
}

fn is_alpha_space(value: &str) -> bool {
    !value.is_empty()
        && value
            .chars()
...
fn booking_total(price_per_hour: f64, time_in: i64, time_out: i64) -> f64 {
    let hours = ((time_out - time_in) as f64 / MS_PER_HOUR).max(MIN_BILLABLE_HOURS);
    (price_per_hour * hours * 100.0).round() / 100.0
}

fn spots_json(spots: &[db::Spot]) -> Json {
    Json::Arr(spots.iter().map(spot_json).collect())
}

fn spot_json(spot: &db::Spot) -> Json {
    json::obj([
        ("_id", json::s(spot.id.as_str())),
        ("city", json::s(spot.city.as_str())),
        ("createdAt", json::i(spot.created_at)),
        ("isAvailable", Json::Bool(spot.is_available)),
        ("latitude", json::n(spot.latitude)),
        ("longitude", json::n(spot.longitude)),
        ("overnight", json::n(spot.overnight)),
        ("price", json::n(spot.price)),
        ("state", json::s(spot.state.as_str())),
        ("streetAddress", json::s(spot.street_address.as_str())),
        ("style", json::s(spot.style.as_str())),
        ("summary", json::s(spot.summary.as_str())),
        ("title", json::s(spot.title.as_str())),
        ("updatedAt", json::i(spot.updated_at)),
        ("userId", json::s(spot.user_id.as_str())),
        ("zipcode", json::s(spot.zipcode.as_str())),
    ])
}

fn booking_json(booking: &db::Booking, spot: Option<&db::Spot>) -> Json {
    let mut body = BTreeMap::new();
    body.insert("_id".to_string(), json::s(booking.id.as_str()));
    body.insert("createdAt".to_string(), json::i(booking.created_at));
    body.insert("spotId".to_string(), json::s(booking.spot_id.as_str()));
    body.insert("status".to_string(), json::s(booking.status.as_str()));
    body.insert("timeIn".to_string(), json::i(booking.time_in));
    body.insert("timeOut".to_string(), json::i(booking.time_out));
    body.insert("total".to_string(), json::n(booking.total));
    body.insert("userId".to_string(), json::s(booking.user_id.as_str()));
    if let Some(spot) = spot {
        body.insert("spot".to_string(), spot_json(spot));
    }
    Json::Obj(body)
}
...
mod tests {
    ...
    fn valid_spot_json() -> &'static str {
        r#"{"title":"Test Driveway","streetAddress":"100 Main St","city":"San Francisco","state":"CA","zipcode":"94123","latitude":37.8,"longitude":-122.4,"price":10,"overnight":20,"style":"Driveway","summary":"A quiet driveway with room for one sedan and easy street access nearby."}"#
    }

    #[test]
    fn spots_list_requires_auth_and_returns_seed() {
        let (state, dir) = test_state();
        let anon = handle(&state, Request::for_test("GET", "/api/spots"));
        assert_eq!(anon.status, 401);

        let signup = signed_up(&state, "parker@example.com");
        let mut req = Request::for_test("GET", "/api/spots");
        cookie_header(&mut req, &signup);
        let res = handle(&state, req);
        assert_eq!(res.status, 200, "{}", String::from_utf8_lossy(&res.body));
        let body = json_body(&res);
        let spots = body.as_arr().expect("array");
        assert_eq!(spots.len(), 5);
        assert_eq!(spots[0].get_str("title"), Some("Mission Street Curb"));
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn spots_mine_is_literal_not_an_id() {
        let (state, dir) = test_state();
        let signup = signed_up(&state, "host@example.com");
        let mut mine = Request::for_test("GET", "/api/spots/mine");
        cookie_header(&mut mine, &signup);
        let mine_res = handle(&state, mine);
        assert_eq!(mine_res.status, 200, "{}", String::from_utf8_lossy(&mine_res.body));
        assert_eq!(json_body(&mine_res).as_arr().expect("array").len(), 0);

        let mut one = Request::for_test("GET", "/api/spots/seed-marina");
        cookie_header(&mut one, &signup);
        let one_res = handle(&state, one);
        assert_eq!(one_res.status, 200, "{}", String::from_utf8_lossy(&one_res.body));
        assert_eq!(json_body(&one_res).get_str("title"), Some("Marina Driveway"));
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn spot_create_requires_csrf_and_owner_can_delete() {
        let (state, dir) = test_state();
        let signup = signed_up(&state, "owner@example.com");
        assert_eq!(signup.status, 201, "{}", String::from_utf8_lossy(&signup.body));
        let owner_id = json_body(&signup).get_str("id").expect("id").to_string();

        let mut bare = Request::for_test("POST", "/api/spots");
        replay_cookies(&mut bare, &[&signup], false);
        bare.set_test_body(valid_spot_json().as_bytes().to_vec());
        let denied = handle(&state, bare);
        assert_eq!(denied.status, 403);

        let mut create = Request::for_test("POST", "/api/spots");
        replay_cookies(&mut create, &[&signup], true);
        ...
        let spot_id = json_body(&created).get_str("_id").expect("id").to_string();
        assert_eq!(json_body(&created).get_str("userId"), Some(owner_id.as_str()));

        let other = signed_up(&state, "other@example.com");
        assert_eq!(other.status, 201);
        let mut del = Request::for_test("DELETE", &format!("/api/spots/{spot_id}"));
        replay_cookies(&mut del, &[&other], true);
        let foreign = handle(&state, del);
        assert_eq!(foreign.status, 404);

        let mut own = Request::for_test("DELETE", &format!("/api/spots/{spot_id}"));
        replay_cookies(&mut own, &[&signup, &created], true);
        let removed = handle(&state, own);
        assert_eq!(removed.status, 204);
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn booking_charges_quarter_hour_minimum() {
        let (state, dir) = test_state();
        let signup = signed_up(&state, "booker@example.com");
        let mut req = Request::for_test("POST", "/api/bookings");
        replay_cookies(&mut req, &[&signup], true);
        req.set_test_body(br#"{"spotId":"seed-marina","timeIn":0,"timeOut":1}"#.to_vec());
        let res = handle(&state, req);
        assert_eq!(res.status, 201, "{}", String::from_utf8_lossy(&res.body));
        assert_eq!(
            json_body(&res).get("total").and_then(Json::as_f64),
            Some(booking_total(4.5, 0, 1))
        );
        assert_eq!(booking_total(10.0, 0, 1), 2.5);
        assert_eq!(booking_total(4.0, 0, 9_000_000), 10.0);
        std::fs::remove_dir_all(&dir).ok();
    }
    ...
    fn short_summary_is_rejected() {
        let (state, dir) = test_state();
        let signup = signed_up(&state, "short@example.com");
        let mut req = Request::for_test("POST", "/api/spots");
        replay_cookies(&mut req, &[&signup], true);
        req.set_test_body(br#"{"title":"Test Driveway","streetAddress":"100 Main St","city":"San Francisco","state":"CA","zipcode":"94123","latitude":37.8,"longitude":-122.4,"price":10,"overnight":20,"style":"Driveway","summary":"too short"}"#.to_vec());
        let res = handle(&state, req);
        assert_eq!(res.status, 400);
        assert_eq!(
