...
// ==== SPOTS DOMAIN (parking marketplace) ====
const spotsExec = makeExec(db);
let spotsReady: Promise<void> | null = null;

/**
 * Lazily ensure Spots/Bookings schema + seed demo inventory once.
 *
 * @returns Promise that resolves when schema is ready
 */
async function ensureSpotsReady(): Promise<void> {
  if (!spotsReady) {
    spotsReady = (async () => {
      await ensureSpotsSchema(spotsExec);
      await seedSpotsIfEmpty(spotsExec, generateUUID);
    })().catch((err) => {
      spotsReady = null;
      throw err;
    });
...
/** GET /api/spots — list available parking spots (auth required for app shell). */
app.get('/api/spots', authMiddleware, async (c) => {
  try {
    await ensureSpotsReady();
    const availableOnly = c.req.query('available') !== '0';
    const spots = await listSpots(spotsExec, { availableOnly });
    return c.json(spots);
  } catch (e) {
    logger.error('List spots failed', { error: errorMessage(e) });
    return c.json({ error: 'Failed to list spots' }, 500);
  }
});

/** GET /api/spots/mine — host's own listings. */
app.get('/api/spots/mine', authMiddleware, async (c) => {
  try {
    await ensureSpotsReady();
    const userID = c.get('userID');
    const spots = await listSpots(spotsExec, { userId: userID, availableOnly: false });
    return c.json(spots);
  } catch (e) {
    logger.error('List my spots failed', { error: errorMessage(e) });
    return c.json({ error: 'Failed to list your spots' }, 500);
  }
});

/** GET /api/spots/:id — single spot. */
app.get('/api/spots/:id', authMiddleware, async (c) => {
  try {
    await ensureSpotsReady();
    const id = c.req.param('id');
    if (!id) return c.json({ error: 'Spot id required' }, 400);
    const spot = await getSpot(spotsExec, id);
    if (!spot) return c.json({ error: 'Spot not found' }, 404);
    return c.json(spot);
  } catch (e) {
    logger.error('Get spot failed', { error: errorMessage(e) });
    return c.json({ error: 'Failed to get spot' }, 500);
  }
});

/** POST /api/spots — create listing (host). */
app.post('/api/spots', authMiddleware, csrfProtection, async (c) => {
  try {
    await ensureSpotsReady();
    const body = await parseJsonBody<Partial<SpotInput>>(c);
    if (!body) return c.json({ error: 'Invalid request body' }, 400);
    const err = validateSpotInput(body);
    if (err) return c.json({ error: err }, 400);
    const spot = await createSpot(spotsExec, c.get('userID'), body as SpotInput, generateUUID);
    return c.json(spot, 201);
  } catch (e) {
    logger.error('Create spot failed', { error: errorMessage(e) });
    return c.json({ error: errorMessage(e) || 'Failed to create spot' }, 500);
  }
});

/** PUT /api/spots/:id — update own listing. */
app.put('/api/spots/:id', authMiddleware, csrfProtection, async (c) => {
  try {
    await ensureSpotsReady();
    const id = c.req.param('id');
    if (!id) return c.json({ error: 'Spot id required' }, 400);
    const body = await parseJsonBody<Partial<SpotInput>>(c);
    if (!body) return c.json({ error: 'Invalid request body' }, 400);
    const updated = await updateSpot(spotsExec, id, c.get('userID'), body);
    if (!updated) return c.json({ error: 'Spot not found or not owned by you' }, 404);
    return c.json(updated);
  } catch (e) {
    const msg = errorMessage(e);
    if (msg.includes('must') || msg.includes('required') || msg.includes('invalid')) {
      return c.json({ error: msg }, 400);
    }
    logger.error('Update spot failed', { error: msg });
    return c.json({ error: 'Failed to update spot' }, 500);
  }
});

/** DELETE /api/spots/:id — soft-delete own listing. */
app.delete('/api/spots/:id', authMiddleware, csrfProtection, async (c) => {
  try {
    await ensureSpotsReady();
    const id = c.req.param('id');
    if (!id) return c.json({ error: 'Spot id required' }, 400);
    const ok = await deleteSpot(spotsExec, id, c.get('userID'));
    if (!ok) return c.json({ error: 'Spot not found or not owned by you' }, 404);
    return c.body(null, 204);
  } catch (e) {
    logger.error('Delete spot failed', { error: errorMessage(e) });
    return c.json({ error: 'Failed to delete spot' }, 500);
  }
});

/** GET /api/bookings — current user's parking history. */
app.get('/api/bookings', authMiddleware, async (c) => {
  try {
    await ensureSpotsReady();
    const bookings = await listBookingsForUser(spotsExec, c.get('userID'));
    return c.json(bookings);
  } catch (e) {
    logger.error('List bookings failed', { error: errorMessage(e) });
    return c.json({ error: 'Failed to list bookings' }, 500);
  }
});

/** POST /api/bookings — Park Here (create booking). */
app.post('/api/bookings', authMiddleware, csrfProtection, async (c) => {
  try {
    await ensureSpotsReady();
    const body = await parseJsonBody<Partial<BookingInput>>(c);
    if (!body) return c.json({ error: 'Invalid request body' }, 400);
    if (!body.spotId || typeof body.timeIn !== 'number' || typeof body.timeOut !== 'number') {
      return c.json({ error: 'spotId, timeIn, and timeOut are required' }, 400);
    }
    const booking = await createBooking(
      spotsExec,
      c.get('userID'),
      { spotId: body.spotId, timeIn: body.timeIn, timeOut: body.timeOut },
      generateUUID
    );
    ...
    logger.error('Create booking failed', { error: msg });
